...
import QuotationCreateModal from './resources/Quotation';
...
export default function ResourceModal({
  ...
  const formSubmit = (data) => handleSubmit(data)

  return (
    <>
      {showForm && <QuotationCreateModal formSubmit={formSubmit} closeModal={e => setShowForm(false)} />}
      <section class='px-4 sm:px-6 lg:px-4 xl:px-6 pt-4 pb-4 sm:pb-6 lg:pb-4 xl:pb-6 space-y-4 flex items-center justify-center flex-col w-full'>
        <div className='sm:w-2/3 md:w-2/3 lg:w-2/3 xl:w-2/3 flex flex-col items-center justify-center'>
          <header class='flex items-center justify-between w-full'>
            <h2 class='text-lg leading-6 font-medium text-black'>{resourceName}</h2>
