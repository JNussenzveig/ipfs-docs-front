...
export default function Header() {
  ...
  return (
    <div class='flex items-center flex-shrink-0 w-full h-16 px-10 bg-white sticky top-0 left-0 z-20 shadow'>
      ...
      />
      <div class='ml-10'>
        {token && AUTH_ITEMS.map(item => (<a class='mx-2 text-sm font-semibold text-indigo-700' onClick={e => router.push(item.path)}>
          {item.label}
        </a>))}
        {!token && UNAUTH_ITEMS.map(item => (<a class='mx-2 text-sm font-semibold text-indigo-700' onClick={e => router.push(item.path)}>
          {item.label}
        </a>))}
      </div>
